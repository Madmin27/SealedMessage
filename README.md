...
## Core Features
- **Condition-based unlocks**: Messages support `TimeOnly`, `PaymentOnly`, `TimeAndPayment`, and current V5.2.1 `TimeOrPayment`.
...
## Architecture
- **Smart contracts**: `contracts/SealedMessageFHE_v521.sol` is the current secure FHE contract. Legacy contracts remain in the repo for historical reads.
...
- **Current V5.2.1-FHE contract**: `0x745642ed1fBff5A901F20836481FEF6414dB5Ba1`
- **V5.2-FHE contract**: `0x3D40Bf66f1EDC0A6ff11808FfE1074A7b079d65A`
...
V5.2.1 is current for new secure messages. It keeps V5.2 OR unlock support, blocks paid-message revoke, requires receiver-only payment, and preserves the core security invariant: only the receiver can receive FHE access to key parts after the on-chain conditions are met.
...
- **Versioned deployments**: The frontend can target legacy V3, legacy V4-FHE, legacy V5-FHE, V5.1-FHE, V5.2-FHE, and the current V5.2.1-FHE deployment through a registry-backed version selector.
...
- `NEXT_PUBLIC_FHE_SECURE_V52_CONTRACT_ADDRESS_SEPOLIA` -> V5.2-FHE deployment (`0x3D40Bf66f1EDC0A6ff11808FfE1074A7b079d65A`)
- `NEXT_PUBLIC_FHE_SECURE_V521_CONTRACT_ADDRESS_SEPOLIA` -> current V5.2.1-FHE deployment (`0x745642ed1fBff5A901F20836481FEF6414dB5Ba1`)
- `NEXT_PUBLIC_ZAMA_RELAYER_URL` -> `https://relayer.testnet.zama.org/v2`

## Deployment & Operations
- **Contract deployment**: Deploy Sepolia contracts with the Hardhat scripts, then register addresses in the frontend env for `NEXT_PUBLIC_CONTRACT_ADDRESS_SEPOLIA` (legacy V3), `NEXT_PUBLIC_FHE_CONTRACT_ADDRESS_ZAMA` (legacy V4-FHE), `NEXT_PUBLIC_FHE_SECURE_CONTRACT_ADDRESS_SEPOLIA` (legacy V5-FHE), `NEXT_PUBLIC_FHE_SECURE_V51_CONTRACT_ADDRESS_SEPOLIA` (V5.1-FHE), `NEXT_PUBLIC_FHE_SECURE_V52_CONTRACT_ADDRESS_SEPOLIA` (V5.2-FHE), and `NEXT_PUBLIC_FHE_SECURE_V521_CONTRACT_ADDRESS_SEPOLIA` (current V5.2.1-FHE).
...
- Create a V5.2.1 secure message, verify the payload/metadata CIDs resolve only to encrypted envelopes, then satisfy the configured conditions and decrypt with the receiver wallet.
