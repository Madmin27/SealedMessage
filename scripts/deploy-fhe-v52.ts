...
async function main() {
  ...
  console.log("\n=== SealedMessageFHE_v52 Deployment ===");
  ...
  console.log("Deploying SealedMessageFHE_v52 contract...");

  const SealedMessageFHE_v52 = await hre.ethers.getContractFactory("SealedMessageFHE_v52");
  const contract = await SealedMessageFHE_v52.deploy();
  ...
  console.log(`\n✅ SealedMessageFHE_v52 deployed to: ${address}`);
  ...
  const filename = `fhe-v52-${hre.network.name}.json`;
  ...
  const artifactPath = path.resolve(
    ...
    "artifacts",
    "contracts",
    "SealedMessageFHE_v52.sol",
    "SealedMessageFHE_v52.json"
  );
  try {
    const artifact = JSON.parse(await fs.readFile(artifactPath, "utf8"));
    await fs.writeFile(
      path.join(deploymentsDir, `fhe-v52-${hre.network.name}-abi.json`),
      JSON.stringify(artifact.abi, null, 2)
    );
    console.log(`📄 ABI saved to: deployments/fhe-v52-${hre.network.name}-abi.json`);
  } catch (err) {
  ...
  console.log(`   NEXT_PUBLIC_FHE_SECURE_V52_CONTRACT_ADDRESS_SEPOLIA=${address}`);
  console.log(`2. Make V5.2 current in frontend deployment registry`);
